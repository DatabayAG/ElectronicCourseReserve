...
class ilElectronicCourseReserveDigitizedMediaImporter
{
			...
			foreach($iter as $file_info)
			{
				...
				if ($valid === true) {
					...
					if (!in_array($parsed_item->getType(), $this->valid_items)) {
						$this->logger->info(sprintf('Type of item (%s) is unknown, skipping item.', $parsed_item->getType()));
						continue;
					}
	...
	protected function createFileItem($parsed_item, $raw_xml)
	{
		...
		{
			$this->logger->info(sprintf('File %s not found for item %s, skipping item creation.', $parsed_item->getItem()->getFile(),  $parsed_item->getLabel()));
			return false;
		}
	}
